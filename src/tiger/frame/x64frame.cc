...
// This access is specific for x64 architecture.
...
public:
  ...
  // Get the address of the in-frame variable. Always return the address so we
  // can store/load from it.
  llvm::Value *ToLLVMVal(llvm::Value *frame_addr_addr) const override;
};

class [[deprecated]] InRegAccess : public Access {
public:
  temp::Temp *reg;
