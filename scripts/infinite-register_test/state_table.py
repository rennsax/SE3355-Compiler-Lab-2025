...
class StateTable(object):
    ...
    def store_mem(self, mem_address, val):
        ...
        # if mem_address == 2097104:
        #     print("Store ", val, self._current_func, self._pc, self.load_reg('%rdx'), self.load_reg('%rcx'))
    ...
    def call(self, label):
        ...
        # print("preapre to call ", label)
        label = label.replace('@PLT', '')
        if label in self._label_table:
            self._func_name_stack.append(self._current_func)
            # print("Call from ", self._current_func,self.load_reg('%rsp'),  end=' ')
            self._current_func = label
            rsp = self.load_reg('%rsp') - 8
            self.store_mem(rsp, self._pc)
            # print("to ",self._current_func, self._pc, self.load_reg('%rdx'), self.load_reg('%rcx'))
            ...
            # print(self.load_mem(arg0),end='mm')
            # print(self.load_mem(arg1),end='mm')
    ...
    def ret(self):
        ...
        # print("return value ", self._current_func, self._pc, self.load_reg('%rax'), self.load_reg('%rdx'), self.load_reg('%rcx'))
